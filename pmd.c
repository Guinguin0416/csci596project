...
/*--------------------------------------------------------------------*/
  ...
  FILE *fp;
  if (sid == 0) { // Only the process with rank 0 opens the file
  ...
  for (stepCount=1; stepCount<=StepLimit; stepCount++) {
    ...
    if (sid == 0) {  // Only the process with rank 0 writes to the file
        write_xyz(fp, stepCount);
    }
...
/* Main loop over x, y & z directions starts--------------------------*/
  ...
  for (kd=0; kd<3; kd++) {
    ...
    for (kdd=0; kdd<2; kdd++) {
      ...
      printf("Rank %d: inode = %d for ku = %d\n", sid, inode, ku); 
       // Check for valid rank (DEBUG)
      if (inode < 0 || inode >= nproc) {
        printf("Rank %d: Invalid inode %d\n", sid, inode);
        MPI_Abort(MPI_COMM_WORLD, 1);
      }
...
// 
void write_xyz(FILE *fp, int timestep) {
    fprintf(fp, "%d\n", n); // n is the number of atoms
    fprintf(fp, "Molecular dynamics simulation at timestep: %d\n", timestep); // Comment line with timestep
    for (int i = 0; i < n; i++) {
        // Assuming the atom type is not known and labeled as 'A'
        fprintf(fp, "A %f %f %f\n", r[i][0], r[i][1], r[i][2]);
    }
}
