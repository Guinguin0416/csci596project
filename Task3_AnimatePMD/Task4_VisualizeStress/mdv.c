...
void ComputeStressTensor() {
    ...
    double sigma, epsilon; // Parameters for Lennard-Jones potential, need to be set accordingly
    int i, j, k, l;
    ...
    for (i = 0; i < nAtom - 1; i++) {
        for (j = i + 1; j < nAtom; j++) {
            ...
            sigma = 1.0; // This should be your actual sigma for Lennard-Jones
            epsilon = 1.0; // This should be your actual epsilon for Lennard-Jones
...
// Function to map the stress magnitude to a color
void MapStressToColor(double magnitude, float color[3]) {
    // Normalize the magnitude to [0, 1]
    int minStress = 0.0; // This should be the minimum stress magnitude
    int maxStress = 60.0; // This should be the maximum stress magnitude
    double normalizedStress = (magnitude - minStress) / (maxStress - minStress);
    // Convert to a color (simple grayscale for now)
    // color[0] = normalized;  // Red
    // color[1] = normalized;  // Green
    // color[2] = normalized;  // Blue
    // You can create a more sophisticated mapping here
    if (normalizedStress < 0.25) {
        // Blue to cyan
        color[0] = 0.0; // Red
        color[1] = 4 * normalizedStress; // Green
        color[2] = 1.0; // Blue
    } else if (normalizedStress < 0.5) {
        // Cyan to green
        color[0] = 0.0; // RedS
        color[1] = 1.0; // Green
        color[2] = 1.0 - 4 * (normalizedStress - 0.25); // Blue
    } else if (normalizedStress < 0.75) {
        // Green to yellow
        color[0] = 4 * (normalizedStress - 0.5); // Red
        color[1] = 1.0; // Green
        color[2] = 0.0; // Blue
    } else {
        // Yellow to red
        color[0] = 1.0; // Red
        color[1] = 1.0 - 4 * (normalizedStress - 0.75); // Green
        color[2] = 0.0; // Blue
    }
}
